  ...
  return (
    <>
      ...
      >
        ...
        {agent.isEnabled && (
          <div className="absolute -right-12 -top-12 h-32 w-32 rounded-full bg-sky-200/40 blur-2xl pointer-events-none group-hover:bg-sky-300/50 transition-colors" />
        )}
        ...
        <div>
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-1">
              {agent.isEnabled ? (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-emerald-500 to-teal-600 px-3.5 py-1 text-xs font-bold text-white shadow-md shadow-emerald-500/20">
                  <Zap className="h-3 w-3 fill-current animate-pulse" />
                  Active
                </span>
              ) : (
                <span className="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-500 border border-slate-200/80">
                  Inactive
                </span>
              )}

              {agent.category && (
                <span className="inline-block rounded-md bg-sky-50 px-2 py-0.5 text-[11px] font-semibold text-[#0284C7] border border-sky-100">
                  {agent.category}
                </span>
              )}
            </div>
