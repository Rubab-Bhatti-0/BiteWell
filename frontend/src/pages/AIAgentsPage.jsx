...
import { Bot, BarChart3, ShieldAlert, Sparkles, Zap, Layers, Activity } from "lucide-react";
...
function AIAgentsPage({ onNavigate }) {
  ...
  const handleDisable = async (agent) => {
    setActiveMutationId(agent.id);
  ...
  return <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-100/60 via-slate-50 to-blue-50/40 p-6 md:p-10 relative overflow-hidden">
      {
  ...
  >
            <button
    onClick={() => onNavigate?.('aiAnalytics')}
    className="flex items-center gap-2 rounded-2xl border border-sky-100 bg-white/90 px-5 py-3 text-sm font-bold text-slate-700 shadow-md shadow-sky-500/5 hover:border-sky-300 hover:bg-sky-50/60 hover:text-[#0284C7] transition-all duration-200 active:scale-95 cursor-pointer"
  >
  ...
  >
    ...
    className={`relative rounded-xl px-5 py-2.5 text-sm font-bold transition-all duration-200 ${selectedCategory === cat ? "bg-gradient-to-r from-[#0284C7] to-[#0A567D] text-white shadow-md shadow-sky-500/25" : "text-slate-600 hover:text-[#0284C7] hover:bg-sky-50/60"}`}
  >
              {cat}
            </button>)}
        </div>
