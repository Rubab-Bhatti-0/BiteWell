...
export function useAgents() {
  ...
  const fetchAgents = useCallback(async () => {
    ...
    try {
      ...
      // Fallback to initial data if backend is offline
      setAgents([
        {
  ...
  const enableAgent = async (agent: Agent) => {
    try {
      const token = await getAuthToken();
      ...
      setAgents((prev) =>
        prev.map((a) => (a.id === agent.id ? { ...a, isEnabled: true } : a))
      );
  ...
  const disableAgent = async (agent: Agent) => {
    try {
      ...
      setAgents((prev) =>
        prev.map((a) => (a.id === agent.id ? { ...a, isEnabled: false } : a))
      );
...
export function useAgentAnalytics() {
  const [data, setData] = useState<AgentAnalytics | undefined>(undefined);
  const [isLoading, setIsLoading] = useState(true);
    ...
    async function loadAnalytics() {
      ...
      try {
        ...
        setData({
          ...
          dailyRequests: (apiData.dailyUsage || []).map((d: any) => ({
            date: d.date,
            requests: d.count
          })),
          usageByAgent: (apiData.usageByAgent || []).map((u: any) => ({
            name: u.name,
